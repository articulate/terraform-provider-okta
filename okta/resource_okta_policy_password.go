	...
	return &schema.Resource{
		...
		Schema: buildPolicySchema(map[string]*schema.Schema{
			...
			"lockout_notification_channels": {
				...
				Description: "If e-mail should be sent when to the user when their account gets locked.",
				Default:     false,
			},
			"question_min_length": {
				Type:        schema.TypeInt,
...
func resourcePolicyPasswordRead(d *schema.ResourceData, m interface{}) error {
	...
	if policy.Settings != nil {
		...
		d.Set("lockout_notification_channels", convertStringSetToInterface(policy.Settings.Password.Lockout.UserLockoutNotificationChannels))
...
func buildPasswordPolicy(d *schema.ResourceData, m interface{}) *articulateOkta.Policy {
	...
	template.Settings.Password.Lockout.UserLockoutNotificationChannels = convertInterfaceToStringSet(d.Get("lockout_notification_channels"))
